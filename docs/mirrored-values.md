...
| the credential redaction families | `swarm_api.redaction.RULES` mirrors `redact()` in `common.sh`; the key/value rule, which takes an escaped quote since #221 and a run of backslashes before it since the PR #229 review, is one Python rule and two sed expressions | `tests/unit/control_plane/test_log_redaction.py`, including the direction that matters — a rule added to the shell filter and not to Python — and, for the key/value rule, both filters RUN over the same JSON-text lines with their output held equal |
...
| the log route's stream names, `stdout`/`stderr` and `agent_stdout`/`agent_stderr` (#184) | `STREAMS` and `AGENT_STREAMS` in `apps/swarm-mcp/swarm_mcp/follow.py` restate `swarm_api.inspect.STREAMS` and `swarm_api.agent_streams.AGENT_STREAMS`. The bridge depends on swarm-common alone, so it cannot import them; a name the route stopped serving would come back as a 422 on every `swarm_follow` row read | `tests/unit/mcp/test_remote_steps_bridge.py::test_the_stream_names_are_the_apis` requires both tuples equal |
| a workflow spec's digest, `fnv1a32:` over its sorted-key compact JSON (PR #230) | `specDigest` and `canonical` in `plugin/workflows/run.js` restate `swarm_mcp.workflows.spec_digest`. A workflow script has no imports and cannot call Python; the two must agree byte for byte, or `swarm_workflow` refuses every `/sc:run` submission as a changed spec | `tests/unit/mcp/test_plugin_agents_and_workflows.py::test_run_js_digests_a_spec_exactly_as_the_bridge_does` runs run.js under node over a spec with non-ASCII, control characters, unsorted nested keys and an integral float, and requires the digest equal to Python's |
| how many calls in a row a follow row may fail to read its task (PR #230) | `READ_FAILURE_LIMIT` in `swarm_mcp/progress.py` is written out as a word in `plugin/agents/step.md`, which a haiku row reads to know when the bridge will stop it; the tool description derives it | `tests/unit/mcp/test_plugin_agents_and_workflows.py::test_the_step_agent_states_the_bridges_read_failure_limit` requires the word to be the constant's |

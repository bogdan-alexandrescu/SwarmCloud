...
| the quota broker's account id in its `swarmSecretLister` grant | since #69 `terraform/bootstrap/platform_roles.tf` makes the grant and names `swarm-quota-broker` itself, because bootstrap cannot read an account `terraform/infra` creates (`modules/iam`) | `tests/terraform/platform_roles.tftest.hcl` requires the grant's member to be `modules/iam`'s `quota_broker_account_id` output, at plan |
| the broker grant's old `for_each` key in the `moved` block that hands it to bootstrap | `terraform/infra/custom_roles_moved_to_bootstrap.tf` spells `swarm-quota-broker:projects/saga-agents-staging/roles/swarmSecretLister` literally, because a `moved` address cannot be computed; a key that matches nothing makes the release DESTROY the grant instead of forgetting it | `tests/terraform/platform_roles.tftest.hcl` requires the literal to be the broker's id and `swarmSecretLister`'s name from `terraform/modules/custom_role_ids`. The custom role ids themselves are **derived**, not compared: both roots read that one module (#79) |
| the marker that says `terraform/infra` has let go of the custom roles | `terraform/infra/custom_roles_moved_to_bootstrap.tf` writes `terraform/bootstrap` as the output `custom_roles_owner`, and `terraform/bootstrap/platform_roles.tf` (`infra_states_holding_platform_roles`) compares each named infra state's output with its own copy of that string. A drift fails closed: the owner's step-2 apply is refused forever | `tests/terraform/platform_roles.tftest.hcl`: `terraform_infra_writes_the_marker_bootstrap_waits_for` requires infra's planned output to be the literal that `adoption_proceeds_once_terraform_infra_has_let_go` hands bootstrap, and that run requires bootstrap to admit it |

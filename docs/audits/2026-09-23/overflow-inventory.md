...
| F9 | **fixed by Lane 1** (#3, `c440e68`) | *Added 2026-09-24 from #3's own record, not re-read here.* The 12px spill was measured against a node DOM that #3's rebuild replaced, and the idle `stop` button sits inside the card by construction. The same defect survived in the control's OPENED states, which this inventory never opened: `.stop-confirm`'s `max-width: 62ch` (~440px) against a 248px node. Bounded to the node's text column, on its own layer. |
...
| F11 | **partly fixed** (#3, `c440e68`, and `styles.css` as it stands) | *Added 2026-09-24, by reading.* Fixed: `.wf-shape-text`, the one genuine escape in the baseline set, is contained at every width, and the 390px `[shape]` column went 56px → 72px so `1 → 5 → 1` survives whole (#3); the workflow id gains a `title` with its complete value (#3); `.wf-progress-text` is `flex: 0 0 auto`, so `1/5 done · 1 not started` no longer absorbs the row's pressure (its comment in `styles.css` names this finding's string). **Not established:** `.wf-state` still declares `overflow: hidden; text-overflow: ellipsis`, so whether `state not derived` — the row's worst case, 25% lost — now fits its column is a layout question no source reading answers. |

...
export function shapeOf(steps: readonly WorkflowStep[]): DagShape {
  const widths = levelsOf(steps).map((l) => l.length)
  const total = steps.length
  const levels = widths.length
  const widest = widths.length === 0 ? 0 : Math.max(...widths)
  const text = widths.join(' → ')

  if (total === 0) return { widths, kind: 'empty', text: '—', label: 'This workflow has no steps.', levels, widest, steps: total }
  if (total === 1) return { widths, kind: 'single', text, label: 'A single step.', levels, widest, steps: total }
  if (widest === 1) {
    return {
      widths,
      kind: 'chain',
  ...
  const label =
    kind === 'diamond'
      ? `A fan-out and a join: ${first} step opens into ${widest} running in parallel, and they converge into ${last}.`
      : kind === 'fan-in'
        ? `A join: ${widest} steps at the widest point converge into ${last}.`
        : kind === 'fan-out'
          ? `A fan-out: ${first} step opens into ${widest} running in parallel.`
          : `${total} steps over ${levels} levels, up to ${widest} in parallel.`

  return { widths, kind, text, label, levels, widest, steps: total }
}

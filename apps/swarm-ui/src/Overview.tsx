...
/* A figure that is REAL but not CURRENT: its window reset, or the poll is past
   ...
   BOTH ARE --text-faint, AND NEITHER IS --ctl-absent. Both used to be spelled
   --ctl-absent, which was then var(--text-faint), so each painted the pixel
   it paints now. --ctl-absent is now its own warm stone (styles.css, :root),
   and it means one thing: NOBODY MEASURED THIS -- the em dash a never-polled
   row gets in this same column. A projected figure WAS measured; it is old
   information, and accountHeadroom above says old information and no
   information are opposite facts. So:
     the bar   a proportion fill is grey unless it carries a verdict
               (design-system.md §6.4, owner decision 2026-09-24), and a
               stone told apart from grey by its hue is not a grey;
     the tilde is the one glyph that says this row is not current, and drawn
               in --ctl-absent it sat dE76 0 from the em dash that says a row
               was never read.
   In the light theme this grey is 1.11:1 from the live bar's --text-dim, so

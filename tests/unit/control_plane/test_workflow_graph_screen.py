...
docstring that must turn it red:
  ...
  3. AN ABSENT MEASUREMENT IS NEVER A NUMBER. The run panel's writing --
     ...
     node carrying a cost is the obvious place to lose it. `stepfacts.ts` makes
...
def test_the_development_fixture_contains_a_fan_in_to_draw():
    ...
    """
    api = _src("api.ts")
    fixture = _decl(api, "async function fixtureWorkflowBoard(")
...
def test_an_absent_cell_has_no_value_to_render():
    ...
    """
    facts = _src("stepfacts.ts")
    union = facts[facts.index("export type Cell<T>") :]
    ...
    assert "word: string" in absent[0] and "note: string" in absent[0]
...
def test_an_absent_cost_renders_the_word_and_never_a_number():
    ...
    """
    wf = _src("Workflows.tsx")
    fact = _decl(wf, "function Fact<T>(")
    absent_branch = _block(fact, "if (cell.kind === 'absent') {")

    assert "{cell.word}" in absent_branch, (
        "the absent branch no longer renders the sentence the absence carries"
    )
    values = re.findall(r'className="node-fact-value">\{([^}]*)\}', absent_branch)
    assert values == ["cell.word"], (
        f"the absent branch renders {values!r} in the value slot. An absent measurement "
        f"must never render as a digit -- $0.00 on five nodes of a six-step workflow is "
        f"a bill nobody owes."
    )
    # And nothing in the absent branch may coalesce to a number.
    assert "?? 0" not in absent_branch and "|| 0" not in absent_branch
...
def test_every_absent_cost_carries_its_own_reason():
    ...
    """
    facts = _src("stepfacts.ts")
    fn = _decl(facts, "export function stepCost(")
    for const in ("COST_NO_RUN", "COST_UNREAD", "COST_RUNNING", "COST_UNREPORTED"):
        assert const in fn, f"stepCost no longer distinguishes {const}"
    assert "?? 0" not in fn and "|| 0" not in fn, "stepCost coalesces an absence to a number"

    # The sentence that does the work. It has to deny the zero out loud,
    # because a reader supplies one otherwise.
    copy = re.search(r"export const COST_UNREPORTED = '([^']*)'", facts)
    assert copy, "COST_UNREPORTED is not a single-quoted constant any more"
...
def test_a_measured_zero_is_still_a_digit():
    ...
    fn = _decl(facts, "export function stepCost(")
    measured = re.search(
        r"if \(typeof raw === 'number' && Number\.isFinite\(raw\)\) \{", fn
    )
    assert measured, (
        "the measured branch no longer accepts every finite number, so a measured zero "
        "may be being turned into an absence"
    )
    assert "raw === 0" not in fn and "raw > 0" not in fn and "raw !== 0" not in fn, (
        "stepCost special-cases zero, which turns a measured zero into an absence"
    )
...
def test_the_run_panel_keeps_the_writing_the_node_borrows():
    ...
    """
    agent = _src("AgentDetail.tsx")
    assert "No attempt reported a cost. This is an absent measurement, not $0.00." in agent
    assert "Written when an attempt ends. A running attempt has none." in agent

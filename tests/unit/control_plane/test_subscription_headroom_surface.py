...
def test_an_age_over_two_days_is_reported_in_days():
    ...
    """
    # types.ts, not Shell.tsx. `timeAgo` MOVED there so `checks.ts` could use it:
    # that module is pure on purpose -- Results in, Check[] out, no React in its
    # import graph -- which is what lets a plain node runner exercise the
    # Overview derivation whose failure mode is silence. Importing it from
    # Shell.tsx would have dragged every component along with it.
    fn = uncommented(body_of(src("types.ts"), "export function timeAgo("))
...
def test_the_accounts_check_still_names_the_never_polled_accounts():
    ...
    """
    fn = uncommented(body_of(src("checks.ts"), "function accountCheck("))
...
def test_no_sentence_on_this_screen_counts_one_account_as_plural():
    ...
    for signature in (
        ...
        fn = uncommented(body_of(source, signature))

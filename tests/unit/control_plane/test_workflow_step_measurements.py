...
def test_a_measured_zero_bar_gets_a_baseline():
    ...
    fill = re.search(r"\n\.ctl-util-fill \{[^}]*\}", css, re.S)
    assert fill is not None
    assert "min-width" not in fill.group(0), (
...
def test_the_data_source_strip_collapses_to_one_line():
    text = src("DataSources.tsx")
    assert "<details" in text and "<summary" in text, "the strip is not a disclosure"
    assert "sl-p95" in text and "percentile(" in text, "the collapsed line reports no p95"
    assert "route{probes.length === 1 ? '' : 's'} read" in text, (
    ...
    assert "source-cells" in text, "the per-route cells were deleted rather than collapsed"
    assert '<a href="#reference">' in text, "there is no route to the full detail"
...
def test_the_reauth_button_is_not_behind_the_disclosure():
    ...
    action, so it sits outside the `<details>` entirely."""
    text = src("DataSources.tsx")
    reauth = text.index('className="reauth"')
    details = text.index('<details className="sources-detail"')
    assert reauth < details, "the re-auth button moved inside the disclosure"
...
def test_the_page_widens_past_1100px():
    ...
    assert caps, ".app has no max-width at all"
    wide = re.findall(r"@media \(min-width: (\d+)px\) \{ \.app \{ max-width: (\d+)px", css)
    assert wide, (
        "the page is still capped at one width. Content ran x=266..1334 on a "
        "1600px viewport -- 250px of dead page on each side."
    )
    assert any(int(cap) > 1100 for _, cap in wide), "no breakpoint widens the page"

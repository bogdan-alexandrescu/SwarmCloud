...
def test_the_graph_emits_one_edge_per_dependency():
    ...
    """
    body = _body(_src("Workflows.tsx"), "edgesOf")
    assert "for (const step of steps)" in body
    assert "for (const parent of step.depends_on)" in body
    assert "edges.push({ from: parent, to: step.step_id })" in body
...
def test_the_graph_renders_an_svg_path_for_every_edge():
    ...
    assert "edges.map((edge) =>" in source
    assert "<path" in source and 'className="dag-edge"' in source
...
def test_the_graph_spends_the_full_content_width():
    ...
    is what spends it.
    """
    row = _rule(_src("styles.css"), ".dag-row > .node")
...
def test_identifiers_opt_out_of_text_transform_through_one_rule():
    ...
    """
    rule = _rule(_src("styles.css"), ".ident")
...
def test_every_identifier_rendered_under_an_uppercasing_rule_carries_it():
    ...
    assert '<span className="ident">{workflow.workflow_id}</span>' in workflows
    assert '<span className="ident">{step.step_id}</span>' in workflows

    # `.tag` uppercases too, and both of these hold identifiers.
    assert 'className="tag ident"' in _src("Agents.tsx")
    assert 'className="tag ident"' in _src("Activity.tsx")
    ...
    assert '<span className="ident">{runtime.name}</span>' in runtimes

...
file pins the parts a process cannot show from the outside:
  ...
  * **The startup Firestore budget** reaches the three generation-check reads
    and the attempt's first write, and nothing else.
...
def test_the_startup_budget_reaches_the_generation_check_and_the_first_write_only():
    ...
    # The mid-run poll keeps the library's defaults: changing how long a
    # running agent tolerates a Firestore outage is a different decision.
    db.calls.clear()
    control.poll(None)
